...
<div class="main-content">
    <div class="page-content">
        ...
        <div id="productList">
                                ...
                                <div class="row g-3">
                                    <div class="col-xxl-3">
                                        <div class="search-box position-relative">
                                            <input type="text" class="form-control search" name="search" placeholder="Search products..." value="{{ request('search') }}" autocomplete="off">
            ...
            <!-- MODAL - FULLY SCROLLABLE + BIDIRECTIONAL + BULK DISCOUNT + VISUAL BADGE -->
                            ...
                            <div class="modal-body" style="max-height: 75vh; overflow-y: auto;">
                                <div class="row g-4">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="row g-3">
                                                    ...
                                                    <div class="col-md-4">
                                                        ...
                                                        <div class="input-group"><span class="input-group-text">$</span><input type="number" step="0.01" name="price" id="price" class="form-control" required></div>
                                                    </div>
                                                    <div class="col-md-4">
                                                        <label>Discount %</label>
                                                        <input type="number" step="0.01" min="0" max="100" id="discount_percent" class="form-control" placeholder="25">
                                                    </div>
                                                    <div class="col-md-4">
                                                        <label>Sale Price</label>
                                                        <div class="input-group"><span class="input-group-text">$</span><input type="number" step="0.01" name="sale_price" id="sale_price" class="form-control"></div>
                                        ...
                                        <div id="variationsSection" style="display:none;">
                                            <div class="card mt-4">
                                                <div class="card-header d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div>
                                                        <input type="number" step="0.01" min="0" max="100" id="bulk_discount" class="form-control form-control-sm d-inline-block w-auto" placeholder="Bulk %">
                                                        <button type="button" class="btn btn-sm btn-success ms-2" id="applyBulkDiscount">Apply to All</button>
                                                    </div>
                                                </div>
                                                <div class="card-body">
                                    ...
                                    <div class="col-lg-4">
                                        ...
                                        <div class="card mb-3">
                                            <div class="card-body">
                                                <label>Gallery (Drag to sort)</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Your original List.js + bulk + search + choices
...
document.getElementById('applyBulkDiscount')?.addEventListener('click', () => {
    ...
    if (bulkDiscount < 0 || bulkDiscount > 100) return Swal.fire('Error', 'Discount must be 0-100%', 'error');
    ...
    document.querySelectorAll('#variationsTable tbody tr').forEach(row => {
        ...
        if (price > 0) {
            saleInput.value = (price - (price * bulkDiscount / 100)).toFixed(2);
            const badge = row.querySelector('.discount-badge') || document.createElement('span');
            badge.className = 'badge bg-danger discount-badge position-absolute';
            badge.textContent = `-${bulkDiscount}%`;
            badge.style.top = '5px'; badge.style.right = '5px'; badge.style.zIndex = '10';
            row.querySelector('td').style.position = 'relative';
            if (!row.querySelector('.discount-badge')) row.querySelector('td').appendChild(badge);
        }
    });
});
...
function renderVariationsTable(combos) {
    ...
    // Add bidirectional pricing to each variation
...
function renderExistingVariations() {
    ...
    productData.variations.forEach((v, i) => {
        ...
        const imgUrl = v.image ? v.image : '';

        html += `<tr>
            <td style="position:relative"><small>${name}</small>
                ${Object.entries(attrs).map(([k,val]) => `<input type="hidden" name="variations[${i}][attributes][${k}]" value="${val}">`).join('')}
                ${v.sale_price && v.price ? `<span class="badge bg-danger discount-badge position-absolute" style="top:5px;right:5px;">-${Math.round(((v.price - v.sale_price) / v.price) * 100)}%</span>` : ''}
            </td>
            ...
            <td>
                <input type="file" name="variations[${i}][image]" class="form-control form-control-sm variation-image" accept="image/*">
                ${imgUrl ? `<img src="${imgUrl}" class="variation-preview mt-2 img-fluid rounded" style="max-height:80px;">` : ''}
            </td>
            <td><button type="button" class="btn btn-danger btn-sm remove-variation">Remove</button></td>
        </tr>`;
    });
...
function resetForm() {
    ...
    document.getElementById('submitBtn').innerHTML = '<span class="spinnerspinner-border spinner-border-sm d-none me-1" id="submitSpinner"></span> Save Product';
